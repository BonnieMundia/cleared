...
- **Payout reversed / bounced.** A record reaching `PAYOUT_ISSUED` then failing at the bank. Model it
  as a terminal money-phase stage in the schema now so no migration is needed later, but do not build
  UI for it.
- **Partial payments.** 60% on approval, the rest later. The log supports splitting a record; no UI
  exists.
