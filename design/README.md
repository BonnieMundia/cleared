...
| File | Contents |
| --- | --- |
| `BUILD_SETUP.md` | One-time machine setup: Android Studio, JDK, `adb`, putting the phone in debugging mode |
...
PROSPECT ──► SUBMITTED ──► IN_REVIEW ──► APPROVED ──► PAYOUT_ISSUED ──► RECEIVED ──► LANDED
 (pre)      └──────────── WORK PHASE ────────────┘   └────────── MONEY PHASE ──────────┘
                              ...
                              └──► REJECTED  (terminal — leaves work phase, never becomes money)
                                                          │
                                                          └──► REVERSED  (terminal — a payout that
                                                               reached the wallet or bank and failed)
```
...
- **Reversed is red too, and terminal.** A payout that bounced. Fees are kept, hours are kept, income
  is zero — the only record whose net contribution can be negative. Recovery is a **new** record
  linked back to it, never a return to an earlier stage. Not a rejection: the platform approved.
- **A record may pay in parts.** Settlements divide the money and never the hours; the effective rate
  lives on the record. A part-paid record shows only its unlanded remainder as owed.
...
| `4a` | **Record detail — reversed** | The chain breaks after Received; kept fees, kept hours, and the linked successor record |
| `4b` | **Record detail — part paid** | Two settlements under one record; split hero bar; the rate stays on the parent |
| `4c` | **Pipeline rows** | Reversed row in a "Needs attention" band; part-paid row with the split encoded in its rail |
...
- **Reversed** — a payout failed after arriving; pinned above the week groups (`4a`, `4c`)
- **Part paid** — some settlements landed, some have not (`4b`, `4c`)
...
- Reversals and partial payments are now designed (turn 4) and folded into `DATA_MODEL.md` and
  `SCREENS.md`. Both are additive: one stage, one nullable pointer, one settlement table.
- **Who supplies the reversal signal?** The design assumes it arrives as an email or SMS the app can
  parse, or is logged by hand. If a platform API can report it, say which.
- **Are settlement fractions per-platform or per-record?** The design shows them as a platform trait
  (`Lumen pays 60 / 40`) but stores them per settlement. Confirm before the schema is fixed.
