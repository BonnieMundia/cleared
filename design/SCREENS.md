...
## `4a` Record detail — payout reversed

Same shell as `1e`. What differs:

**Header** — stage chip reads `Reversed` in `rejectedContainer` / `onRejectedContainer`. Hero figure
`KES 20,323` in `onSurface`, **not** in the rejected colour: the money exists, it is just in the wrong
place. Caption `Sitting in Payoneer since 18 Jul · never reached Equity Bank`. The third stat cell is
`Cleared` / `KES 0` in `onRejectedContainer` — the one red figure on the screen.

**History** — work phase violet as usual, but only two nodes: Halo Data reports approval directly, so
no `IN_REVIEW` event is ever written and the timeline draws `Submitted → Approved`. This is correct —
render whatever events exist, never a placeholder for a stage that was skipped. Money phase green
through `Payout issued` and
`Received in Payoneer`. The connector **below** `Received` becomes a 1.5 dp **dashed red** line. The
final node is an 11 dp **hollow** red ring (2 dp stroke, surface fill), not a filled dot: the step did
not complete. Label `Reversed by the bank` at 13.5 sp 600 in `onRejectedContainer`, then the reason in
a `rejectedContainer` block at 11.5 sp / 1.45.

**Money ledger** — as `1e`, with `not refunded` as an 11 sp sub-label under commission and withdrawal
fee, and a `Return handling fee` line. Footer row is `Back in the wallet`, not `Cleared`. Summary
block in `rejectedContainer`: the cost, then the reminder that the hours stay.

**Re-issued as** — a 12 dp outlined card, green rail, mono ref, stage chip, amount, and `0.0 h`.
Tapping it opens the successor record. Below it, one line: `Carries no hours of its own — the work was
already counted here.` Absent when no re-issue exists yet; in its place, a text button
`Log the re-issue`.

---

## `4b` Record detail — part paid

**Header** — chip `Part paid` in `moneyContainer`. Hero is the record's **whole** KES value in
`onSurface`. Under it a 6 dp split bar, 2 dp gap, 3 dp radius: cleared portion `money`, remainder
`moneyContainer`. Then a two-column line — cleared KES in `onMoneyContainer` left, in-flight KES in
`onSurfaceVariant` right. Stat row is Gross · Hours logged · **Effective** — the rate belongs here and
nowhere else on this screen.

**Settlements** — section label with the platform's terms on the right (`Lumen pays 60 / 40`). One
12 dp outlined card per settlement: green rail, label at 14 sp 500, stage chip, a mono timing line
(`Issued 18 Jul → landed 22 Jul · 4 d` for landed; `Issued 2 Aug → expected 8 Aug · p90 11 d` for
in-flight), amount and KES right-aligned, chevron. Cards are tappable — they open a settlement-scoped
copy of the `1e` ledger. Footer line: `Only the second settlement counts as owed. The first has left
the pipeline.`

**Shared work phase** — one violet timeline for the record. The connector below `Approved` is a
violet → green gradient. The last node is `Split into 2 settlements` with a
`surfaceContainer` block stating the hours rule. No per-settlement work phase exists.

---

## `4c` Pipeline rows for both states

**Reversed row.** Red rail. Platform name in `onSurface`, **not struck through** — struck means the
money never existed, which is the `REJECTED` case, not this one. Chips: `Reversed` in
`rejectedContainer`, then an `overdueContainer` age pill reading `17d stalled`. Right column: gross in
`onSurface`, and `KES 1,412 lost` in `onRejectedContainer` where the KES figure normally sits.

**Needs attention band.** Reversed and conflicted records pin to a group **above** the week groups,
header `Needs attention` in `onOverdueContainer` with a count on the right. Same 8 dp band treatment
as a week header. The band is absent when empty — never shown with a zero.

**Part-paid row.** The rail encodes the split: a linear gradient, `money` to the cleared fraction,
`moneyContainer` below it. Chip reads `Part paid · 40% left`. The row shows the **remaining**
settlement's amount and KES, not the record total, because the row lives under the week the remainder
is expected — and its week subtotal must add up.
