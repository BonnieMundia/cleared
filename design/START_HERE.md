...
You are on `github.com/BonnieMundia`. This walks from an empty machine to an APK on your phone and a
...
Then in any folder, run `claude` and follow the login prompt once.

You also need **Android Studio** (for the SDK, the emulator, and to open the project) and a **JDK 17**.
Android Studio bundles a JDK, so installing it is usually enough.

---

## 2. Create the repo and lay out the files
...
## 3. Open the design before you start
...
- "Design the payout-reversed state" — one of the three gaps flagged in `CLAUDE.md`.
